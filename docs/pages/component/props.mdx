...
On iOS (15+) AVPlayer has no explicit variant selector: `resolution` / `index` are applied as a cap through `AVPlayerItem.preferredMaximumResolution`, so the player will use the requested variant or a lower one under poor bandwidth. `disabled` is not supported on iOS.
...
Errors caused by the network being unavailable (DNS failure, connection refused/reset, timeouts) are not counted: the player keeps retrying every second until connectivity comes back. Only errors the server answers with (HTTP 4xx/5xx) give up after `minLoadRetryCount` attempts.

...
| deviceName               | string \| null | Name of the route playing the video while it is active (`null` when playing locally) |
...
| isLive      | boolean | `true` for live streams (HLS/DASH without a fixed duration)                                                                                                                                                                                                                                                                                                       |
...
| isLive           | boolean | `true` for live streams (HLS/DASH without a fixed duration)           |
| liveOffset       | number | Seconds behind the live playback position (`0` = live); `-1` when not live or unknown |
...
Triggered when video tracks change, and when the ABR switches to another variant (the `selected` flag is refreshed). On iOS (15+) the tracks are the HLS variants of the master playlist.
